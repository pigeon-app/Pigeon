// Note:
// This code is adapted from v0.1.0+2 of
// https://pub.dartlang.org/packages/scrollable_bottom_sheet, but includes
// some miscellaneous bug fixes:
// - the correct minimum height is used for the callback
// - halfHeight can be passed manually into ScrollableBottomSheetByContent
...
class ScrollableBottomSheetByContent extends StatefulWidget {
  ...
  ScrollableBottomSheetByContent(this.header, this.content,
      {ScrollableController controller, bool snapAbove, bool snapBelow, double halfHeight, bool autoPop, this.callback, ScrollState scrollTo})
      : this.controller = controller ?? ScrollableController(),
        this.snapAbove = snapAbove ?? true,
        this.snapBelow = snapBelow ?? true,
        this.halfHeight = halfHeight ?? 0.0,
    ...
    Widget child = LayoutBuilder(builder: (BuildContext context, BoxConstraints constraints) {
      ...
      if (_currentHeight == null) {
        _currentHeight = widget.firstScrollTo == ScrollState.half
            ? _halfHeight
            : widget.firstScrollTo == ScrollState.full ? _fullHeight : _minimumHeight;
      } else if (_currentHeight < _minimumHeight) {
          ...
          child: GestureDetector(
              onVerticalDragEnd: (DragEndDetails details) {
                ...
                if (_currentHeight >= _fullHeight && _currentState != ScrollState.full) {
                  ...
                  if (_postFrame && widget.callback != null) widget.callback(_currentState);
                  ...
                  _currentState = ScrollState.half;
                  if (_postFrame && widget.callback != null) widget.callback(_currentState);
                } else if (_currentHeight <= _minimumHeight && _currentState != ScrollState.minimum) {
        ...
        setState(() {
          ...
          if (zeroScale.value == targetHeight && zeroScale.status == AnimationStatus.completed) {
            if (targetHeight == _fullHeight) {
              ...
              _currentState = ScrollState.half;
            } else if (targetHeight == _minimumHeight) {
              _currentState = ScrollState.minimum;
            }
